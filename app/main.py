...
st.set_page_config(
    page_title="FleetOps AI | XCMG Innovation Track",
    page_icon="🚧",
    layout="wide",
)
...
def style_fig(fig, height=380):
    ...
    fig.update_xaxes(
        gridcolor="#30363D", linecolor="#484F58", tickfont=dict(color="#C9D1D9")
    )
    fig.update_yaxes(
        gridcolor="#30363D", linecolor="#484F58", tickfont=dict(color="#C9D1D9")
    )
...
with tab_overview:
    ...
    with c1:
        kpi_card("Fleet size", report["fleet_size"])
    with c2:
        kpi_card("Overall utilization", f"{report['overall_utilization_%']}%")
    with c3:
        kpi_card(
            "Total operating hours", f"{report['total_operating_hours']:,.0f}"
        )
    with c4:
        kpi_card("Total fuel (L)", f"{report['total_fuel_l']:,.0f}")
    ...
    with col1:
        ...
        top_downtime = pd.DataFrame(
            tools.get_top_downtime_equipment(8)["data"]
        )
        fig = px.bar(
            top_downtime,
            x="equipment_id",
    ...
    if attention.empty:
        ...
        st.dataframe(
            attention[["equipment_id", "equipment_type", "project", "reason"]],
    ...
    st.caption(
        "Combines downtime + fuel deviation + maintenance frequency + ML anomaly detection into one 0-100 score"
    )
    scores = pd.DataFrame(ml_anomaly.compute_attention_scores()["data"])
    fig3 = go.Figure(
        go.Bar(
            x=scores["equipment_id"],
            y=scores["attention_score"],
            marker=dict(
                color=scores["attention_score"],
                colorscale=[[0, STEEL], [0.5, GOLD], [1, RED]],
            ),
            text=scores["attention_score"],
...
with tab_chat:
    ...
    st.caption(
        "Try: 'Which equipment has the highest downtime?' / 'Which equipment needs urgent attention?' "
...
with tab_report:
    ...
    st.markdown(
        f"**Overall utilization rate:** {report['overall_utilization_%']}%"
    )

    st.markdown("#### Performance by project")
    st.dataframe(
        pd.DataFrame(report["by_project"]),
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.dataframe(
        pd.DataFrame(report["top_downtime_equipment"]),
        use_container_width=True,
        hide_index=True,
    )
    ...
    if report["fuel_anomalies"]:
        ...
        st.dataframe(
            pd.DataFrame(report["fuel_anomalies"]),
            use_container_width=True,
            hide_index=True,
        )
...
with tab_upload:
    ...
    st.download_button(
        "⬇️ Download a sample equipment_master.csv (as a template)",
        open("data/equipment_master.csv", "rb").read(),
        file_name="equipment_master_template.csv",
    )
    st.download_button(
        "⬇️ Download a sample fleet_daily_logs.csv (as a template)",
        open("data/fleet_daily_logs.csv", "rb").read(),
        file_name="fleet_daily_logs_template.csv",
    )

    st.divider()
    up_equip = st.file_uploader(
        "Upload equipment_master.csv", type="csv", key="up_equip"
    )
    up_daily = st.file_uploader(
        "Upload fleet_daily_logs.csv", type="csv", key="up_daily"
    )
    ...
    if up_equip and up_daily:
        try:
            ...
            if missing_e or missing_d:
                if missing_e:
                    st.error(
                        f"equipment_master.csv is missing required columns: {sorted(missing_e)}"
                    )
                if missing_d:
                    st.error(
                        f"fleet_daily_logs.csv is missing required columns: {sorted(missing_d)}"
                    )
            else:
                ...
                if not st.session_state.get("custom_data_active", False):
                    ...
                    st.rerun()
                st.success(
                    f"✅ Loaded {new_equip['equipment_id'].nunique()} units, "
