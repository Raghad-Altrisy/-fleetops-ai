...
def build_pdf_report(report):
    """Build a compact downloadable fleet-performance PDF from the current report."""
    try:
    ...
    doc = SimpleDocTemplate(
        buffer, pagesize=A4, rightMargin=36, leftMargin=36,
        topMargin=36, bottomMargin=36
    )
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle(
        "FleetOpsTitle", parent=styles["Title"], alignment=TA_CENTER,
        textColor=colors.HexColor(RED), spaceAfter=12
    )
    small = ParagraphStyle("Small", parent=styles["BodyText"], fontSize=8, leading=10)

    story = [
        Paragraph("FleetOps AI — Fleet Performance Report", title_style),
        Paragraph(f"<b>Period:</b> {html.escape(str(report.get('period', 'N/A')))}", styles["BodyText"]),
        Paragraph(f"<b>Fleet size:</b> {report.get('fleet_size', 'N/A')}", styles["BodyText"]),
        Paragraph(f"<b>Overall utilization:</b> {report.get('overall_utilization_%', 'N/A')}%", styles["BodyText"]),
        Paragraph(f"<b>Total operating hours:</b> {report.get('total_operating_hours', 'N/A')}", styles["BodyText"]),
        Paragraph(f"<b>Total fuel:</b> {report.get('total_fuel_l', 'N/A')} L", styles["BodyText"]),
        Spacer(1, 12),
        Paragraph("Performance by Project", styles["Heading2"]),
    ]

    project_rows = [["Project", "Operating Hours", "Downtime Hours", "Utilization %"]]
    for r in report.get("by_project", []):
        project_rows.append([
            str(r.get("project", "")), str(r.get("operating_hours", "")),
            str(r.get("downtime_hours", "")), str(r.get("utilization_%", "")),
        ])
    t = Table(project_rows, repeatRows=1)
    ...
    story.extend([t, Spacer(1, 12), Paragraph("Top Downtime Issues", styles["Heading2"])])

    downtime_rows = [["Equipment", "Type", "Project", "Downtime %"]]
    for r in report.get("top_downtime_equipment", []):
        downtime_rows.append([
            str(r.get("equipment_id", "")), str(r.get("equipment_type", "")),
            str(r.get("project", "")), str(r.get("downtime_rate_%", "")),
        ])
    t2 = Table(downtime_rows, repeatRows=1)
    t2.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,0), colors.HexColor(RED)),
        ("TEXTCOLOR", (0,0), (-1,0), colors.white),
        ("GRID", (0,0), (-1,-1), 0.4, colors.HexColor("#D8DCE1")),
        ("FONTSIZE", (0,0), (-1,-1), 8),
        ("ROWBACKGROUNDS", (0,1), (-1,-1), [colors.white, colors.HexColor("#FFF6F7")]),
    ]))
    story.append(t2)

    actions = report.get("manager_actions", [])
    if actions:
        story.extend([Spacer(1, 12), Paragraph("Recommended Management Actions", styles["Heading2"])])
        for i, action in enumerate(actions, 1):
            story.append(Paragraph(f"{i}. {html.escape(str(action))}", small))
    ...
    story.extend([
        ...
        Paragraph(
            ...
            "Track: AI + Construction Machinery → AI + Operations", small
        ),
        Paragraph("Built by: Raghad Altrisy • Taif Alharbi • Jood Alwajeeh", small),
    ])
    doc.build(story)
...
with st.sidebar:
    ...
    api_mode = st.selectbox(
        "AI Mode", ["Test Mode (Free)", "Real Mode (OpenAI API)"],
        help="Test Mode uses the built-in local agent. Real Mode uses the configured OpenAI API."
    )
    if api_mode == "Real Mode (OpenAI API)":
        ...
        if api_key:
            ...
            st.success("✅ Real Mode enabled")
        else:
            os.environ.pop("OPENAI_API_KEY", None)
            st.warning("Enter an API key to enable Real Mode.")
    else:
        os.environ.pop("OPENAI_API_KEY", None)
    ...
    top_n = st.slider("Top downtime equipment", 3, 15, 8,
                      help="Controls how many units appear in the downtime chart.")
    downtime_palette = st.selectbox(
        "Downtime chart scale",
        ["Low → High (Steel → Red)", "Low → High (Green → Yellow → Red)"],
    ...
    st.caption("15th China International College Students' Innovation & Entrepreneurship Competition")
...
with tab_overview:
    ...
    with col1:
        ...
        top_downtime = pd.DataFrame(tools.get_top_downtime_equipment(top_n)["data"])
        downtime_scale = (
            [[0, GREEN], [0.5, GOLD], [1, RED]]
            if downtime_palette.startswith("Low → High (Green")
            else [[0, STEEL], [0.5, RED], [1, RED_DARK]]
        )
        fig = px.bar(
            top_downtime, x="equipment_id", y="downtime_rate_%",
    ...
    with col2:
        ...
        fig2 = px.bar(
            by_project, x="project", y="utilization_%",
...
with tab_report:
    ...
    csv_bytes = pd.DataFrame(report["by_project"]).to_csv(index=False).encode("utf-8-sig")
    json_bytes = json.dumps(report, ensure_ascii=False, indent=2, default=str).encode("utf-8")
    pdf_bytes = build_pdf_report(report)

    ec1, ec2, ec3 = st.columns(3)
    with ec1:
        st.download_button("📊 CSV", csv_bytes, file_name="fleet_performance_report.csv",
                           mime="text/csv", use_container_width=True)
    with ec2:
        st.download_button("📄 JSON", json_bytes, file_name="fleet_performance_report.json",
                           mime="application/json", use_container_width=True)
    with ec3:
        if pdf_bytes:
            st.download_button("📋 PDF", pdf_bytes, file_name="fleet_performance_report.pdf",
...
<div class="xcmg-footer">
    ...
    Track: AI + Construction Machinery → AI + Operations<br><br>
    Built by: Raghad Altrisy • Taif Alharbi • Jood Alwajeeh<br>
    📊 Live Dashboard &nbsp;|&nbsp; 🔌 REST API &nbsp;|&nbsp; 🤖 AI-Powered &nbsp;|&nbsp; 🚀 Production Ready
</div>
""", unsafe_allow_html=True)
